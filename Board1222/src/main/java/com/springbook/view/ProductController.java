...
//		
	...
	public String productDelete(ProductVO vo, Model model) throws Exception {
		System.out.println("=====> Controller productDelete.do");
		//자바스크립트에서 넘어온 값을 디코딩
		String decoderStr = URLDecoder.decode(vo.getProduct_imgStr(), "UTF-8");
		File delF = new File(path + decoderStr);
		if (!vo.getProduct_imgStr().equals("space.png")) {
			System.out.println("=====> Controller 파일삭제");
			delF.delete(); // 실제파일 삭제
		}
...
//		
	...
	public String productUpdateOK(@ModelAttribute("m") ProductVO vo, Model model) throws Exception {
		System.out.println(" ====> path:" + path);
